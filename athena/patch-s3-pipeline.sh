#!/bin/bash
# Patches the alloy-logs configmap to add the debug → S3 pipeline.
# Re-run this after every helm upgrade.
set -e

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PIPELINE_FILE="$SCRIPT_DIR/s3-pipeline.alloy"

CURRENT=$(kubectl get configmap grafana-k8s-monitoring-alloy-logs -n monitoring -o jsonpath='{.data.config\.alloy}')

if echo "$CURRENT" | grep -q "pods_s3"; then
  echo "S3 pipeline already present, skipping."
  exit 0
fi
